...
namespace Shiroi.FX.Effects.BuiltIn {
    ...
    public class AudioEffect : Effect {
        ...
        private void PlayWithController(PositionFeature position, EffectContext context, AudioClip clip,
            ...
            if (position == null) {
                audioEvent = new AudioEvent(clip, pitch, volume, Loop, loopDuration, Range, Mixer,
                    context.Host.transform);
            } else {
                var velFeature = context.GetOptionalFeature<VelocityFeature>();
                var velocity = velFeature?.Velocity ?? Vector3.zero;
                audioEvent = new AudioEvent(clip, pitch, volume, position.Position, Loop, loopDuration, Range, Mixer,
                    velocity);
            }
